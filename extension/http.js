...
if (!chrome.sockets || !chrome.sockets.tcpServer) {
  throw new Error("!chrome.sockets || !chrome.sockets.tcpServer")
  return {};
}
...
var PSocket = (function() {
  ...
  chrome.sockets.tcp.onReceive.addListener(function(info) {
    console.log('chrome.sockets.tcp.onReceive', info)

    var pSocket = socketMap[info.socketId];
    if (pSocket) {
      if (pSocket.handlers) {
        console.log('chrome.sockets.tcp.onReceive - pSocket.handlers')
  ...
  chrome.sockets.tcp.onReceiveError.addListener(function(info) {
    console.log('chrome.sockets.tcp.onReceiveError')

    var pSocket = socketMap[info.socketId];
    if (pSocket) {
      if (pSocket.handlers) {
        console.log('chrome.sockets.tcp.onReceiveError - pSocket.handlers')
  ...
  return function(socketId) {
    console.log('function(socketId)')
    this.socketId = socketId;
    this.readQueue = [];

    this.fnGetInfo = (cb) =>
    {
      chrome.sockets.tcp.getInfo(socketId, cb)
    }
...
PSocket.prototype.read = function() {
  console.log('PSocket.prototype.read', this.readQueue)
  var that = this;
  if (this.readQueue.length) {
    console.log('PSocket.prototype.read this.readQueue.length')
...
PSocket.prototype.write = function(data) {
  console.log('PSocket.prototype.write')
  var that = this;
  return new Promise(function(resolve, reject) {
    chrome.sockets.tcp.send(that.socketId, data, function(info) {
      console.log('chrome.sockets.tcp.send')
...
PSocket.prototype.close = function() {
  console.log('PSocket.prototype.close')
  var that = this;
  return new Promise(function(resolve, reject) {
    chrome.sockets.tcp.disconnect(that.socketId, function() {
      console.log('chrome.sockets.tcp.disconnect')
...
var arrayBufferToString = function(buffer) {
  ...
  console.log('arrayBufferToString - str', str)
  return str;
};
...
EventSource.prototype = {
  ...
  dispatchEvent: function(type, var_args) {
    console.log('dispatchEvent', type, var_args)
    if (!this.listeners_[type])
      return false;
...
HttpServer.prototype = {
    ...
    chrome.sockets.tcpServer.create(function(socketInfo) {
      ...
      chrome.sockets.tcpServer.listen(
        ...
        function(result) {
          ...
          cb(result, chrome.runtime.lastError)
  ...
  readRequestFromSocket_: function(pSocket) {
    console.log('readRequestFromSocket_', pSocket)
    var t = this;
    var requestData = '';
    var endIndex = 0;
    var onDataRead = function(data) {
      console.log('onDataRead', data)
    ...
    pSocket.read().then(onDataRead).catch(function(e) {
      console.log('onDataRead catch', e.message)
      console.error(e)
      pSocket.close();
    });
  },
  ...
  onRequest_: function(request) {
    console.log('onRequest_', request)
...
HttpRequest.prototype = {
  ...
  write_: function(array) {
    ...
    this.pSocket_.write(array).then(function(bytesWritten) {
      ...
      console.log('this.pSocket_.write(array)')
      console.error(e);
      return;
    });
  },
...
WebSocketServer.prototype = {
  ...
  upgradeToWebSocket_: function(request) {
    console.log('upgradeToWebSocket_', request)
...
function WebSocketRequest(httpRequest) {
  console.log('WebSocketRequest', httpRequest)
...
function WebSocketServerSocket(pSocket) {
  console.log('WebSocketServerSocket', pSocket)
  this.pSocket_ = pSocket;
  this.readyState = 1;
  EventSource.apply(this);
...
WebSocketServerSocket.prototype = {
  ...
  readFromSocket_: function() {
    ...
    var onDataRead = function(dataBuffer) {
      console.log('onDataRead', dataBuffer)
    ...
    this.pSocket_.read().then(function(data) {
      console.log('this.pSocket_.read().then', data)
      return onDataRead(data);
    }).catch(function(e) {
      console.log('this.pSocket_.read().catch')
      console.error(e)
      t.close_();
    });
  },
  ...
  sendFrame_: function(op, data) {
    console.log('sendFrame_', op, data)
    var t = this;
  ...
  close_: function() {
    console.log('close_', this.readyState)
